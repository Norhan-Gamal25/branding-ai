...
_GOLD_SCAFFOLD = """
=== REQUIRED PAGE STRUCTURE (7 sections in order) ===
Produce a complete <!DOCTYPE html> document matching this structure exactly.
Replace ALL_CAPS placeholders with real brand content. No Lorem ipsum.

HEAD: Tailwind CDN + Phosphor Icons CDN + Google Fonts @import + <style> with:
  :root { --base:#HEX; --surface:#HEX; --primary:#HEX; --secondary:#HEX; --text-light:#HEX; --text-muted:#HEX; }
  .g-text { gradient text using primary->secondary }
  .card { glassmorphism: rgba(PRIMARY_RGB,0.08) backdrop-blur border-radius:20px hover:translateY(-4px) }
  .btn-primary { pill shape, var(--primary) bg, glow box-shadow, hover:translateY(-2px) }
  nav { sticky, backdrop-blur, var(--base) at 88% opacity }
  tailwind.config = { theme:{ extend:{ colors:{ primary,secondary,base,surface }, fontFamily:{ display,body } } } }

BODY sections (each with DIFFERENT background, alternating --base / --surface):
1. <nav> — logo (Phosphor icon + brand name + tagline), links: About/Features/Contact, CTA button
2. <section id="hero"> — radial CSS glow, eyebrow pill, h1.g-text clamp(2.8rem,6vw,4.5rem), subheadline, .btn-primary
3. Trust bar — 3 stat+icon items in a flex row using real business facts only
4. <section id="about"> — 2-col grid: text (headline+2 paragraphs) | 2x2 .card grid with Phosphor icons
5. <section id="features"> — section headline + 6 .card items each with icon box + title + 2-sentence desc
6. <section id="contact"> — large Phosphor icon, .g-text headline, subtext, mailto .btn-primary
7. <footer> — brand name + tagline + copyright

RULES: No <img>, no raw <svg> shapes, no Lorem ipsum, no invented reviews/ratings.
All icons from Phosphor: <i class="ph ph-ICONNAME"></i>
=== END STRUCTURE ===
"""
